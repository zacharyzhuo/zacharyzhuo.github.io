...
- **Q 彈動效**：iOS26 風格彈簧統一用 `--ease-spring` token（Tailwind 對應 `ease-spring`）。可點離散元件（卡片 / 選單項 / CTA / icon 按鈕）套 `.press-springy`（按下 squish、放開彈回）或共用的 `.frosted-glass-button`。**勿無差別灑到所有 transition**：進度條、fade、骨架、列表整列、docked 面板（sheet/抽屜會 overshoot 露縫）不套。`prefers-reduced-motion` 由全域 reset 自動降為瞬時
