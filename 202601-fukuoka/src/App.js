...
import { 
  ...
  Utensils, ShoppingBag, Train, ChevronRight, Navigation, ClipboardList, Check, Info, AlertCircle, ExternalLink, Luggage, Menu, Clock, Bath, BookOpen
} from 'lucide-react';
...
const checklistData = {
  ...
  "Toiletries": ["化妝品", "防曬", "暖暖包", "洗面乳", "飾品（項鍊/耳環/戒指）", "保養品", "口罩"],
  "Health": ["定期需要吃的藥"],
...
const Sidebar = ({ isOpen, onClose, onSelect }) => {
  return (
    <>
      ...
      />
      <div 
        className={`fixed inset-y-0 left-0 w-64 bg-[#FDFBF9] shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <nav className="space-y-2 px-4">
            ...
            >
              ...
              <div>
                <span className="block font-serif font-bold text-jp-text text-sm">旅程資訊</span>
                <span className="block text-[10px] text-stone-400 font-sans tracking-wide">Flight & Info</span>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <span className="block font-serif font-bold text-jp-text text-sm">行李清單</span>
                <span className="block text-[10px] text-stone-400 font-sans tracking-wide">Packing List</span>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <span className="block font-serif font-bold text-jp-text text-sm">逛街清單</span>
                <span className="block text-[10px] text-stone-400 font-sans tracking-wide">Shopping Map</span>
              </div>
            </button>
          </nav>
        </div>
        ...
        <div className="p-6 border-t border-stone-100 bg-stone-50">
          <p className="text-[10px] text-stone-400 text-center font-sans tracking-widest uppercase">Family Trip 2026</p>
        </div>
      </div>
    </>
  );
    ...
    <div className="flex justify-between items-center px-6 py-4 border-b border-stone-200/50 bg-jp-bg sticky top-0 z-10">
      {days.map((day) => {
        ...
        return (
          ...
          >
            <span className={`text-[10px] tracking-widest uppercase font-sans ${isActive ? 'text-jp-red font-bold' : 'text-stone-400'}`}>
              {weekDay}
            </span>
            <span className={`text-2xl font-serif leading-none ${isActive ? 'text-jp-text' : 'text-stone-300'}`}>
              {dayNum}
            </span>
...
const ActivityItem = ({ activity, isLast, onOpen }) => {
  const getIcon = (type) => {
    switch (type) {
      ...
      case '美食': return <Utensils size={16} />;
      case '購物': return <ShoppingBag size={16} />;
      case '景點': return <Camera size={16} />;
      case '住宿': return <Hotel size={16} />;
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border border-stone-100 active:scale-[0.98] transition-transform duration-200 h-full flex flex-col">
          <div className="flex justify-between items-start mb-2">
            <span className={`text-[10px] tracking-wider uppercase px-2 py-0.5 rounded border font-sans font-bold ${
              ...
              activity.type === '交通' ? 'border-blue-200 text-blue-700 bg-blue-50' : 
              activity.type === '購物' ? 'border-pink-200 text-pink-700 bg-pink-50' : 
              ...
              {activity.subItems.map((item, idx) => (
                <div key={idx} className="flex flex-col gap-0.5 text-xs font-sans">
                  <div className="flex justify-between items-baseline">
                    <span className="font-bold text-stone-600">{item.title}</span>
                    {item.hours && <span className="text-[10px] text-stone-400">{item.hours}</span>}
                  </div>
                  {item.desc && <span className="text-[10px] text-stone-400 line-clamp-1">{item.desc}</span>}
                </div>
              ))}
            </div>
          )}
  ...
  return (
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 transform animate-slide-up">
        ...
        <div className="bg-[#FDFBF9] rounded-t-[2rem] p-8 pb-10 shadow-2xl min-h-[60vh] max-h-[90vh] overflow-y-auto relative">
          ...
          <h2 className="text-3xl font-serif font-bold text-jp-text mb-2 leading-tight">
          ...
          <div className="space-y-8">
                                    ...
                                    {activity.showShoppingLink && (
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center gap-3">
                                          ...
                                          <span className="font-bold text-pink-700 text-sm">{activity.shoppingText || '查看天神逛街地圖'}</span>
            ...
            {activity.tips && (
              ...
              <div>
                ...
                <h3 className="text-[10px] font-bold text-stone-400 mb-3 uppercase tracking-[0.2em] font-sans border-b border-stone-100 pb-2">Traveler Tips / 旅人攻略</h3>

                <div className="flex flex-wrap gap-2">

                  {activity.tips.map(tip => (

                    <span key={tip} className="px-4 py-2 bg-white border border-stone-200 rounded-full text-xs text-stone-600 shadow-sm font-sans">
          ...
          <div className="sticky bottom-0 mt-12 pt-4 bg-gradient-to-t from-[#FDFBF9] via-[#FDFBF9] to-transparent">

            <button 

              onClick={() => {

                const url = activity.nav.startsWith('http') ? activity.nav : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(activity.nav)}`;
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 transform animate-slide-up">
        <div className="bg-[#FDFBF9] rounded-t-[2rem] p-8 pb-10 shadow-2xl min-h-[60vh] max-h-[90vh] overflow-y-auto relative">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-serif font-bold text-jp-text">旅程資訊</h2>
            <button onClick={onClose} className="p-2 bg-stone-100 rounded-full text-stone-500">
              <X size={20} />
            </button>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100">
                  <div className="flex justify-between items-center mb-4 border-b border-stone-50 pb-2">
                    <span className="text-xs font-bold text-stone-400 bg-stone-50 px-2 py-1 rounded">去程 01/10</span>
                    <span className="text-xs font-bold text-jp-green">AirAsia AK 1510</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="text-center">
                      <div className="text-2xl font-serif font-bold text-jp-text">12:40</div>
                      <div className="text-sm text-stone-400 font-sans">TPE 台北</div>
                    </div>
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-serif font-bold text-jp-text">16:00</div>
                      <div className="text-sm text-stone-400 font-sans">FUK 福岡</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100">
                  <div className="flex justify-between items-center mb-4 border-b border-stone-50 pb-2">
                    <span className="text-xs font-bold text-stone-400 bg-stone-50 px-2 py-1 rounded">回程 01/14</span>
                    <span className="text-xs font-bold text-jp-green">AirAsia AK 1511</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="text-center">
                      <div className="text-2xl font-serif font-bold text-jp-text">16:55</div>
                      <div className="text-sm text-stone-400 font-sans">FUK 福岡</div>
                    </div>
                    <div className="flex flex-col items-center text-stone-300">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-serif font-bold text-jp-text">18:30</div>
                      <div className="text-sm text-stone-400 font-sans">TPE 台北</div>
                    </div>
                  </div>
                </div>
              ...
              >
                 ...
                 <div className="relative z-10 flex items-center justify-between">
                   <div>
                     ...
                     <p className="text-xs opacity-80 font-sans tracking-wide">入境日本必備・提前填寫申報</p>
                   </div>
                   <ExternalLink size={20} className="opacity-80 group-hover:opacity-100" />
                 </div>
              </button>
            </div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 transform animate-slide-up">
        <div className="bg-[#FDFBF9] rounded-t-[2rem] p-8 pb-10 shadow-2xl min-h-[60vh] max-h-[90vh] overflow-y-auto relative">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-serif font-bold text-jp-text">行李清單</h2>
            <button onClick={onClose} className="p-2 bg-stone-100 rounded-full text-stone-500">
              <X size={20} />
            </button>
          </div>
                  ...
                  {items.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-sm font-serif transition-all ${
...
const ShoppingModal = ({ isOpen, onClose, initialTab }) => {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50" onClick={onClose} />
      <div className="fixed inset-x-0 bottom-0 z-50 transform animate-slide-up">
        <div className="bg-[#FDFBF9] rounded-t-[2rem] p-8 pb-10 shadow-2xl min-h-[70vh] max-h-[90vh] overflow-y-auto relative flex flex-col">
          
          {/* Header */}
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-serif font-bold text-jp-text">逛街地圖</h2>
            <button onClick={onClose} className="p-2 bg-stone-100 rounded-full text-stone-500">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex gap-2 p-1 bg-stone-100 rounded-xl mb-6">
            <button 
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'tenjin' ? 'bg-white shadow text-jp-text' : 'text-stone-400'}`}
            >
              天神 Tenjin
            </button>
            <button 
              onClick={() => setActiveTab('hakata')}
              className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'hakata' ? 'bg-white shadow text-jp-text' : 'text-stone-400'}`}
            >
              博多 Hakata
            </button>
          </div>
          ...
          <div className="space-y-4 pb-8">
            {currentList.map((item, idx) => (
              <div key={idx} className="bg-white rounded-xl p-5 border border-stone-100 shadow-sm">
                  ...
                  <div className="mt-4 space-y-3 pl-2 border-l-2 border-stone-100">
                    {item.shops.map((shop, sIdx) => (
                      <div key={sIdx} className="flex justify-between items-center group">
                        <div>
                          <p className="font-bold text-sm text-stone-700">{shop.name}</p>
                          <div className="flex items-center gap-2 text-xs text-stone-400 font-sans">
                            <span className="bg-stone-100 px-1.5 py-0.5 rounded text-[10px]">{shop.floor}</span>
                ...
                {!item.isBuilding && (
                  <div className="mt-2 flex items-center gap-3 text-xs text-stone-500 font-sans pl-11">
                    ...
                    <span className="flex items-center gap-1"><Clock size={12} /> {item.hours}</span>
                  </div>
                )}
    ...
    <div className="min-h-screen bg-jp-bg text-jp-text font-serif pb-12">
      ...
      <div className="relative pt-8 pb-4 px-6 flex items-center justify-between">
        ...
        <div className="flex flex-col items-center">
          <p className="text-[10px] tracking-[0.4em] uppercase text-stone-400 mb-1 font-sans font-bold">Family Trip</p>
          <div className="flex items-center justify-center gap-3">
            <h1 className="text-2xl font-bold tracking-[0.2em] text-jp-text">九州旅行</h1>
            <span className="px-2 py-0.5 border border-stone-300 rounded-full text-[10px] text-stone-400 font-sans font-bold">2026/01</span>
          </div>
        </div>
